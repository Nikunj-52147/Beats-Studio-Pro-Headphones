...
const Hero = ({ imgRef, readRef, comfortRef }, ref) => {
 const isLaptop = useBreakpoint();
    return isLaptop ? <Laptop imgRef={imgRef} readRef={readRef} comfortRef={comfortRef} ref={ref}/> : <Mobile imgRef={imgRef} readRef={readRef} comfortRef={comfortRef} ref={ref}/>
};
...
const Mobile = forwardRef(({ imgRef, readRef, comfortRef }, ref) => {
  return (
    <section id="section1" ref={ref} className="bg-black h-screen bg-linear-[346deg,#fdfdfc_45%,#18191a_45%]">
      ...
      <div className="flex flex-col pl-7 pt-7 gap-10">
        <div className="">
          <h1 className="heading text-7xl font-semibold text-[#fcfcfe]">Music Everywhere</h1>
          <h1 className="heading text-7xl font-semibold text-[#e1d8d3]">Anytime</h1>
          <div className="h-0.5 w-[35%] bg-[#78797d] mt-8"></div>
          <div ref={readRef} className="mt-7 cursor-pointer text-xl font-semibold uppercase text-[#b7b9bb]">
           Read More
          </div>
        ...
        <div ref={comfortRef} className="mt-74 w-full flex flex-col gap-5 text-left pr-15 text-neutral-900">
          <h2 className="font-bold text-4xl">All-day Comfort</h2>
          <p className="text-lg">
            The <b>upgraded UltraPlush over-ear cushions</b> feature seamless
            engineered leather, delivering all-day comfort and exceptional
          ...
          <div className="w-fit  px-5 py-1.5 rounded-3xl border-2 border-[#78797d] cursor-pointer z-99">
            <div className="inline-block text-center font-bold">Add to Cart</div>
            <div className="bg-neutral-900 h-2 w-2 rounded inline-block ml-2 text-center"></div>
          </div>
        </div>
