...
const Product_Information = ({s2, sec2ref}) => {
 const isLaptop = useBreakpoint();
    return isLaptop ? <Laptop s2={s2} sec2ref={sec2ref}/> : <Mobile s2={s2} sec2ref={sec2ref}/>
};

const Laptop = ({s2, sec2ref}) => {
  return (
...
const Mobile = ({s2, sec2ref}) => {
  return (
    <section id="section2" ref={s2} className="bg-[#18191a] text-white min-h-screen pl-7 py-7 flex justify-center items-center">
      <div className="">
        <div className="w-full p-2">
          <div ref={sec2ref} className="flex flex-col gap-3 text-xl">

         
          <h2 className="text-6xl font-bold pb-5">Product Information</h2>
          ...
          <div className="w-fit mt-7 px-5 py-1.5 rounded-3xl bg-neutral-800 border-2 border-[#78797d] text-xl  cursor-pointer">
            <div className="inline-block text-center font-bold">Add to Cart</div>
            <div className="bg-white h-2 w-2 rounded inline-block ml-2 text-center"></div>
          </div>
        </div>
